...
class Dataset:
    ...
    def shuffle(self, new_epoch_id):
        ...
        if os.environ.get("MILES_BALANCE_SANDBOX_PROJECTS") == "1":
            # Agentic SWE dual-project runs have equal primary/secondary
            # partitions. Shuffle within each project, then interleave them so
            # every even-sized prompt batch consumes both Sandbox pools evenly.
            # This is opt-in and leaves every other Miles dataset unchanged.
            by_project = {"primary": [], "secondary": []}
            for index, sample in enumerate(self.origin_samples):
                project = sample.metadata.get("sandbox_project", "primary")
                if project not in by_project:
                    raise ValueError(f"unsupported sandbox_project while balancing: {project!r}")
                by_project[project].append(index)
            if len(by_project["primary"]) != len(by_project["secondary"]):
                raise ValueError(
                    "MILES_BALANCE_SANDBOX_PROJECTS=1 requires equal primary/secondary partitions; "
                    f"got primary={len(by_project['primary'])} secondary={len(by_project['secondary'])}"
                )
            rng.shuffle(by_project["primary"])
            rng.shuffle(by_project["secondary"])
            permutation = list(
                itertools.chain.from_iterable(zip(by_project["secondary"], by_project["primary"]))
            )
        else:
