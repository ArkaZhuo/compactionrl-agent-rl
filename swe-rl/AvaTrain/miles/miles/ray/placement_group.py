...
def _create_placement_group(num_gpus):
    ...
    timeout_sec = int(os.getenv("MILES_PLACEMENT_GROUP_TIMEOUT_SEC", "300"))
    try:
        ray.get(pg.ready(), timeout=timeout_sec)
    except ray.exceptions.GetTimeoutError as exc:
        raise RuntimeError(
            f"Timed out after {timeout_sec}s waiting for a {num_gpus}-GPU placement group. "
