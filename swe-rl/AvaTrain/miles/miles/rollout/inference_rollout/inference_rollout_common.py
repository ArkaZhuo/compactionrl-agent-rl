...
async def generate_and_rm_group(
    ...
    try:
        ...
        # sibling tasks. Agentic siblings own external Sandboxes, so detached
        # siblings leak capacity and may keep grading after their group has
        # already been rejected. Unwind every sibling and let its generate
        # function execute Sandbox cleanup in ``finally``.
        for task in tasks:
            if not task.done():
                task.cancel()
