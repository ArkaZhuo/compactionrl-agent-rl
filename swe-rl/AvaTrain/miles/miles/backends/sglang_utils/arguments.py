...
def normalize_args(args):
    """Normalize SGLang CLI fields across supported SGLang releases.

    Miles validates a small number of SGLang options before the full SGLang
    validation pass.  Some SGLang releases keep the corresponding ServerArgs
    fields but no longer expose every option through ``add_cli_args``.  Supply
    the ServerArgs defaults when argparse did not create those attributes so
    Miles can apply its colocate policy consistently.
    """
    for name in ("disable_piecewise_cuda_graph", "enforce_piecewise_cuda_graph"):
        attr = f"sglang_{name}"
        if not hasattr(args, attr):
            setattr(args, attr, getattr(ServerArgs, name, False))


def validate_args(args):
    normalize_args(args)
    args.sglang_tp_size = args.rollout_num_gpus_per_engine
    # SGLang <=0.5.12 exposed the long names below; newer releases use the
    # canonical dp_size/pp_size/ep_size fields directly. The CLI wrapper adds
    # the sglang_ prefix in both cases, so accept either spelling while keeping
    # the canonical fields consumed by the rest of Miles.
    args.sglang_dp_size = getattr(
        args,
        "sglang_data_parallel_size",
        getattr(args, "sglang_dp_size", getattr(ServerArgs, "dp_size", 1)),
    )
    args.sglang_pp_size = getattr(
        args,
        "sglang_pipeline_parallel_size",
        getattr(args, "sglang_pp_size", getattr(ServerArgs, "pp_size", 1)),
    )
    args.sglang_ep_size = getattr(
        args,
        "sglang_expert_parallel_size",
        getattr(args, "sglang_ep_size", getattr(ServerArgs, "ep_size", 1)),
    )
    ...
    elif not hasattr(args, "sglang_attn_cp_size"):
        args.sglang_attn_cp_size = getattr(ServerArgs, "attn_cp_size", 1)
