...
@pytest.mark.parametrize('offload_fraction', [0.0, 0.4, 1.0])
def test_precision_aware_state_dict_with_native_fp32_param(offload_fraction):
    """A precision-aware checkpoint can contain both BF16 and native FP32 params."""

    source_params = [
        nn.Parameter(torch.tensor([1.0, 2.0], device="cuda", dtype=torch.bfloat16)),
        nn.Parameter(torch.tensor([3.0, 4.0], device="cuda", dtype=torch.float32)),
    ]
    source = HybridDeviceOptimizer(
        source_params,
        offload_fraction=offload_fraction,
    ...
    target_params = [
        nn.Parameter(torch.zeros(2, device="cuda", dtype=torch.bfloat16)),
        nn.Parameter(torch.zeros(2, device="cuda", dtype=torch.float32)),
    ]
    target = HybridDeviceOptimizer(
        target_params,
        offload_fraction=offload_fraction,
        cpu_optimizer_cls=Adam,
        gpu_optimizer_cls=Adam,
    ...
    for source_param, target_param in zip(source_params, target_params):
        source_inner = source.param_to_inner_param[source_param]
        target_inner = target.param_to_inner_param[target_param]
        assert torch.equal(target_inner, source_inner)
