...
def test_dp_reshardable_parameter_state_excludes_adam_step():
    """Adam step is checkpointed through param_groups, never bucket tensors."""
    optimizer = object.__new__(DistributedOptimizer)
    model_param = object()
    optimizer.per_bucket_numel = [2]
    optimizer.per_bucket_numel_unpadded = [2]
    ...
    optimizer._get_main_param_and_optimizer_states = mock.Mock(
        return_value={
            "param": torch.zeros(2),
    ...
    assert "step" not in state[0][torch.float32][0][0]


def test_dp_reshardable_load_ignores_parameter_shaped_adam_step():
    """Salvage old checkpoints whose padding expanded Adam step to a vector."""
    optimizer = object.__new__(DistributedOptimizer)
    model_param = object()
    optimizer.per_bucket_numel_unpadded = [2]
    optimizer.gbuf_ranges = [
    ...
    optimizer._set_main_param_and_optimizer_states = mock.Mock()
    parameter_state = {
        0: {
            torch.float32: [
                [
    ...
    optimizer.load_parameter_state_from_dp_reshardable(parameter_state)

    loaded_state = optimizer._set_main_param_and_optimizer_states.call_args.args[1]
    assert "step" not in loaded_state
