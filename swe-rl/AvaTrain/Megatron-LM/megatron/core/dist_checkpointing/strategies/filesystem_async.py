        ...
        **kwargs,
        ...
        try:
            ...
            if use_msc:
                ...
                open_file = open
                attempts = int(os.environ.get("MCORE_DIST_CKPT_WRITE_ATTEMPTS", "3"))
                if attempts < 1:
                    raise ValueError("MCORE_DIST_CKPT_WRITE_ATTEMPTS must be positive")
            ...
            for attempt in range(1, attempts + 1):
                ...
                try:
                    with open_file(write_path, "wb") as stream:
                        for write_item, data in bytes_data:
                            # PyTorch planners normally provide BytesIO objects.
                            # Rewind them so a retry never resumes from a cursor
                    ...
                    if not use_msc:
                        os.replace(write_path, file_name)
                        temporary_path = None
                    break
                except Exception:
                    ...
                    if attempt == attempts:
                        raise
                    logger.warning(
                        "Checkpoint shard write failed (attempt %d/%d); retrying %s",
                        attempt,
                        attempts,
                        file_name,
                        exc_info=True,
