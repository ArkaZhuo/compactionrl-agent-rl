...
class TorchDistSaveShardedStrategy(AsyncSaveShardedStrategy):
    ...
    def __init__(
        ...
        env_thread_count = os.environ.get("MCORE_DIST_CKPT_THREAD_COUNT")
        if env_thread_count is not None:
            try:
                thread_count = int(env_thread_count)
            except ValueError as exc:
                raise ValueError(
                    "MCORE_DIST_CKPT_THREAD_COUNT must be a positive integer"
                ) from exc
        if thread_count < 1:
            raise ValueError("checkpoint thread_count must be positive")
        if separation_hint is not None and thread_count < 2:
            raise ValueError("checkpoint thread_count must be at least 2 with separation_hint")
