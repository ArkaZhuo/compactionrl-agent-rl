...
class HybridDeviceOptimizer(torch.optim.Optimizer):
    ...
    def _update_fp32_params_by_new_state(self):
        ...
        for param, v in self.state.items():
            # ``param_to_fp32_param`` only contains low-precision parameters for
            # which HDO created an FP32 master copy. Native FP32 parameters are
            # absent from that mapping, but their checkpoint state still has a
            # ``master_param`` entry. Use the mapping shared by all parameters so
            # native FP32 parameters resolve to themselves (or their CPU copy
            # when offloaded).
            inner_param = self.param_to_inner_param[param]
