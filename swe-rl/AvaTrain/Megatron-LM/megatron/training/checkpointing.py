...
def _remove_incomplete_torch_dist_checkpoint(checkpoint_name):
    """Remove a failed torch_dist checkpoint directory before re-saving it."""
    if MultiStorageClientFeature.is_enabled():
        return
    if not os.path.isdir(checkpoint_name) or os.path.islink(checkpoint_name):
        return
    ...
    print_rank_0(
        f"  [{datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}] removing incomplete "
        f"torch_dist checkpoint directory {checkpoint_name}"
    )
    ...
    if not torch.distributed.is_initialized() \
        ...
        if ckpt_type == CheckpointType.GLOBAL and ckpt_format == "torch_dist":
            if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
                # A failed save can leave distcp shards without metadata.
                _remove_incomplete_torch_dist_checkpoint(checkpoint_name)
