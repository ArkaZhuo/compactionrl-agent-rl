...
ACTION="${1:-check}"
case "${ACTION}" in
  check|smoke-4gpu|train-4gpu|train-8gpu|train-secondary-8gpu|resume-secondary-8gpu|resume-4gpu|resume-8gpu) ;;
  *) echo "usage: $0 {check|smoke-4gpu|train-4gpu|train-8gpu|train-secondary-8gpu|resume-secondary-8gpu|resume-4gpu|resume-8gpu}" >&2; exit 2 ;;
esac
...
if [[ "${ACTION}" == resume-4gpu || "${ACTION}" == resume-8gpu ]]; then
  ...
  # Dedicated overrides make an explicit recovery command reproducible while
  # keeping arbitrary stale LOAD_CHECKPOINT exports from changing the run.
  export LOAD_CHECKPOINT="${PPO_RESUME_ACTOR_CHECKPOINT:-${SHARED_ROOT}/swe-rl/checkpoints/swe_dev_dual_qwen35_4b_ppo_actor_20260805_133102}"
  export CRITIC_LOAD_CHECKPOINT="${PPO_RESUME_CRITIC_CHECKPOINT:-${SHARED_ROOT}/swe-rl/checkpoints/swe_dev_dual_qwen35_4b_ppo_critic_20260805_133102}"
  ...
  export OPTIMIZER_CPU_OFFLOAD_FRACTION="${PPO_RESUME_OPTIMIZER_CPU_OFFLOAD_FRACTION:-0.1}"
...
if [[ "${ACTION}" == train-8gpu ]]; then
  [[ -z "${LOAD_CHECKPOINT:-}" && -z "${CRITIC_LOAD_CHECKPOINT:-}" ]] || {
    echo "train-8gpu starts from iteration 0 and does not accept load checkpoints" >&2
    exit 2
  }
fi
...
PPO_RESUME_SAVE_ROOT="${PPO_RESUME_SAVE_ROOT:-}"
...
if [[ "${ACTION}" == train-8gpu || "${ACTION}" == resume-8gpu || \
      "${ACTION}" == train-secondary-8gpu || \
...
[[ -z "${PPO_RESUME_SAVE_ROOT}" || "${PPO_RESUME_SAVE_ROOT}" == /* ]] || \
  fail "PPO_RESUME_SAVE_ROOT must be an absolute path"
...
echo "Reward KL penalty  : kl_coef=${KL_COEF}"
...
if [[ -n "${PPO_RESUME_SAVE_ROOT}" ]]; then
  echo "Resume save policy : new lineage under ${PPO_RESUME_SAVE_ROOT}"
fi
...
if [[ -n "${LOAD_CHECKPOINT:-}" || -n "${CRITIC_LOAD_CHECKPOINT:-}" ]]; then
  ...
  elif [[ -n "${PPO_RESUME_SAVE_ROOT}" ]]; then
    mkdir -p "${PPO_RESUME_SAVE_ROOT}"
    [[ -w "${PPO_RESUME_SAVE_ROOT}" ]] || fail "resume save root is not writable: ${PPO_RESUME_SAVE_ROOT}"
    kl_tag="${KL_COEF//./p}"
    export SAVE_DIR="${PPO_RESUME_SAVE_ROOT}/${DATASET_TAG}_qwen35_4b_ppo_actor_resume_from${actor_iteration}_kl${kl_tag}_${RUN_TS}"
    export CRITIC_SAVE_DIR="${PPO_RESUME_SAVE_ROOT}/${DATASET_TAG}_qwen35_4b_ppo_critic_resume_from${critic_iteration}_kl${kl_tag}_${RUN_TS}"
    [[ ! -e "${SAVE_DIR}" ]] || fail "actor resume save directory already exists: ${SAVE_DIR}"
    [[ ! -e "${CRITIC_SAVE_DIR}" ]] || fail "critic resume save directory already exists: ${CRITIC_SAVE_DIR}"
  else
