...
ACTION="${1:-check}"
case "${ACTION}" in
  check|rollout-1gpu|train-4gpu) ;;
  *) echo "usage: $0 {check|rollout-1gpu|train-4gpu}" >&2; exit 2 ;;
esac
...
# Use the audited 500/500 project split by default so the secondary sandbox
# capacity participates without requiring a per-launch override.
...
case "${TRAIN_DATASET}" in
  ...
  verified)
    ...
    ;;
  *) echo "TRAIN_DATASET must be 'swe-dev', 'swe-dev-dual', or 'verified'" >&2; exit 2 ;;
esac
...
case "${ACTION}" in
  rollout-1gpu)
    ...
    if [[ -z "${DEBUG_ROLLOUT_DATA:-}" ]]; then
      ...
      DEBUG_ROLLOUT_DATA="${SHARED_ROOT}/swe-rl/smoke/verified_rollout_"'{rollout_id}.pt'
      export DEBUG_ROLLOUT_DATA
    fi
    task_file="${SMOKE_DATA}"
    ;;
  train-4gpu)
    [[ "$("${PYTHON_BIN}" -c 'import torch; print(torch.cuda.device_count())')" -ge 4 ]] || fail "four visible GPUs are required"
    export NUM_GPUS=4
    ...
    # gradient-bearing actor forward. TP=2 shards that peak while retaining
    # DP=2 and the same global batch/training hyperparameters.
    ...
    # Match the upstream Qwen3.5-4B recipe. The previous 20480-token dynamic
    # microbatch cap could transiently consume more than 78 GiB during the
    # FP32 vocabulary loss and OOM on an otherwise healthy H100 run.
    export MAX_TOKENS_PER_GPU="${MAX_TOKENS_PER_GPU:-9216}"
    ...
    if [[ -n "${LOAD_CHECKPOINT:-}" ]]; then
      ...
      export SAVE_DIR="${SAVE_DIR:-${SHARED_ROOT}/swe-rl/checkpoints/${DATASET_TAG}_qwen35_4b_4gpu_${RUN_TS}}"
    fi
...
if [[ "${ACTION}" == rollout-1gpu ]]; then
  ...
  echo "READY: 4GPU Agentic SWE GRPO run completed (num_rollout=${NUM_ROLLOUT}) and checkpoint output exists."
fi
echo "Log: ${LOG_FILE}"
