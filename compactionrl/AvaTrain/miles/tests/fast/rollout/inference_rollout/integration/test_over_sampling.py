...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            id="bounded_rejections",
        )
    ],
    indirect=True,
)
def test_compaction_rollout_fuse_bounds_rejected_refills(rollout_env, monkeypatch):
    env = rollout_env

    def reject_all(_args, _samples, **_kwargs):
        return DynamicFilterOutput(keep=False, reason="test_reject_all")

    monkeypatch.setenv("COMPACTION_ROLLOUT_MAX_ATTEMPTS", "8")
    monkeypatch.setenv("COMPACTION_ROLLOUT_MAX_DISCARDED", "8")
    with function_registry.temporary("test:reject_all", reject_all):
        with pytest.raises(RuntimeError, match="CompactionRL rollout fuse"):
            load_and_call_train(env.args, env.data_source)

    assert len(env.mock_server.request_log) == 8
