...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def init(
        ...
        if role == "critic":
            ...
            if self.args.critic_ppo_epochs is not None:
                if self.args.critic_ppo_epochs <= 0:
                    raise ValueError("--critic-ppo-epochs must be positive")
                self.args.ppo_epochs = self.args.critic_ppo_epochs
        else:
            for m in all_replay_managers:
                m.enabled = getattr(self.args, f"use_{m.name}_replay")
                m.enable_check_replay_result = m.enabled and self.args.ci_test

        # Megatron caches distributed-checkpoint metadata and save strategies in
        # this mapping.  Keep one context for the actor lifetime instead of
        # rebuilding the collective-heavy save strategy at every checkpoint.
        self._checkpointing_context = {}
        (self.model, self.optimizer, self.opt_param_scheduler, loaded_rollout_id) = initialize_model_and_optimizer(
            args, role, checkpointing_context=self._checkpointing_context
        )
    ...
    def save_model(self, rollout_id: int, force_sync: bool = False) -> None:
        ...
        save(
            ...
            self.optimizer,
            self.opt_param_scheduler,
            checkpointing_context=self._checkpointing_context,
        )
