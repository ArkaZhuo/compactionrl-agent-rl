...
def forward_only(
    ...
    def forward_step(
        ...
        output_tensor = model(
            ...
            # Keep the full vocabulary output in model precision.  Converting
            # the packed [tokens, vocab] tensor to fp32 here creates a very
            # large transient allocation; log-prob postprocessing upcasts its
...
def train_one_step(
    ...
    def forward_step(
        ...
        if return_schedule_plan:
            ...
            # Match Megatron's RL forward path: retain fp16/bf16 vocabulary
            # logits and let the chunked log-prob processor do fp32 math.
...
def train(
    ...
    for global_step_id in range(total_steps):
        ...
        if step_id == 0 and ppo_epoch > 0:
            # PPO reuses the same long trajectories for multiple optimization
            # epochs.  Qwen3.5's large vocabulary can leave multi-GiB inactive
            # CUDA allocator blocks after the first pass; TorchMemorySaver does
            # not support expandable_segments, so release cached blocks at the
            # epoch boundary before constructing the next backward graph.
            clear_memory()
            if is_megatron_main_rank():
                logger.info("Cleared CUDA allocator cache before PPO epoch %d", ppo_epoch + 1)
...
def save(
    ...
    Args:
        ...
        optimizer (MegatronOptimizer): Optimizer instance.
        opt_param_scheduler (OptimizerParamScheduler): LR/WD scheduler.
        checkpointing_context (dict | None): Mutable Megatron checkpoint cache
            shared across saves in this worker.
    """
    args = get_args()
    hashes = None
...
def initialize_model_and_optimizer(
    args: Namespace, role: str = "actor", checkpointing_context: dict | None = None
    ...
    Args:
        ...
        role (str): Logical role of the model (e.g., "actor", "critic").
        checkpointing_context (dict | None): Mutable Megatron checkpoint cache
            shared by loading and subsequent saves in this worker.
