...
class UpdateWeightFromTensor:
    ...
    def connect_rollout_engines(
        ...
        """
        # Keep the complete engine set for lifecycle operations.  In mixed
        # colocated/distributed mode ``self.rollout_engines`` is narrowed below
        # to the colocated engines used by the IPC weight-update path.
    ...
    def update_weights(self) -> None:
        ...
        uses_weight_update_session = False
        ...
        if rank == 0:
            ...
            if not skip_base_sync:
                begin_results = ray.get(
                    [engine.begin_weight_update.remote(selector="all") for engine in self.rollout_engines]
                )
                support_flags = [
                    bool(result.get("supported")) if isinstance(result, Mapping) else False
                    for result in begin_results
                ]
                if any(support_flags) and not all(support_flags):
                    raise RuntimeError(
                        "Rollout engines expose a mixed SGLang weight-update protocol; "
                        "all engines must run the same SGLang version."
                    )
                uses_weight_update_session = all(support_flags)
                _check_weight_sync_results(begin_results, is_lora=False)
                logger.info(
                    "SGLang transactional weight-update protocol: %s",
                    "enabled" if uses_weight_update_session else "legacy fallback",
                )

            if (
                not skip_base_sync
                and not uses_weight_update_session
        ...
        if rank == 0:
            if not skip_base_sync:
                if uses_weight_update_session:
                    end_results = ray.get([engine.end_weight_update.remote() for engine in self.rollout_engines])
                    _check_weight_sync_results(end_results, is_lora=False)
                else:
                    # Legacy SGLang uses a one-shot bf16 → int4/Marlin
                    # post-processing endpoint instead of begin/update/end.
                    post_process_weights(
                        rollout_engines=self.rollout_engines,
                        restore_weights_before_load=False,
                        post_process_quantization=True,
                    )
            # release_memory_occupation() pauses every rollout engine before
            # offload.  Resume the complete set after both the colocated IPC
            # and distributed NCCL weight updates have finished.
