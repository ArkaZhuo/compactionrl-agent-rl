...
class SGLangEngine(RayActor):
    ...
    def begin_weight_update(self, selector: str = "all"):
        """Open the transactional weight-update protocol when SGLang supports it.

        SGLang added this protocol after the Miles version pinned by AvaTrain.
        Older servers return 404 and continue to use Miles' legacy
        post_process_weights lifecycle.
        """
        if self.node_rank != 0:
            return {"success": True, "supported": True}

        url = f"http://{self.server_host}:{self.server_port}/begin_weight_update"
        response = requests.post(url, json={"selector": selector})
    ...
    def end_weight_update(self):
        """Close a transactional weight-update session on newer SGLang."""
    ...
    def abort_all_requests(self):
        """Abort requests that outlived their rollout client."""
        if self.node_rank != 0:
            return
        response = requests.post(
    ...
    def flush_cache(self, timeout_seconds: float = 60, retry_interval_seconds: float = 1):
        """Wait for the server to become idle, then flush its cache."""
        if self.node_rank != 0:
            return
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError(
                    f"Timeout while flushing cache after {timeout_seconds}s "
    ...
    def release_memory_occupation(self, tags: list[str] = None):
        ...
        # pause_generation(abort) closes the admission gate, aborts outstanding
        # requests, and waits for SGLang's model-update lock. A bare abort_request
        # is fire-and-forget and can race a late prefill with cache reset/offload.
        self.pause_generation(mode="abort", timeout_seconds=120)
        self.flush_cache()
        return self._make_request(
    ...
    def pause_generation(self, mode: str = "retract", timeout_seconds: float = 600):
        response = requests.post(
