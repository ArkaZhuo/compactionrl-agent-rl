...
def sync_actor_critic_data(
    ...
    if not values:
        # LinearForLastLayer returns critic values in fp32. Log-probs are only
        # a shape/device template here; their dtype is not the wire contract.
        # This matters for CP ranks with zero local response tokens, whose empty
        # policy output can otherwise retain the bf16 model dtype.
        values = [torch.empty_like(log_prob, dtype=torch.float32) for log_prob in log_probs]
    for value in values:
