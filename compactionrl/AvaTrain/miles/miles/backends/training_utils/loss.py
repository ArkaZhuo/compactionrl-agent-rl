...
def compute_advantages_and_returns(args: Namespace, rollout_data: RolloutBatch) -> None:
    ...
    # Intermediate PP stages have no post-forward tensors. A CP peer can also
    # legitimately have no response-aligned tensors when a short response lies
    # entirely on another CP rank; custom advantage functions still need that
    # last-stage peer to participate in their collectives.
    if log_probs is None and values is None:
        if not get_parallel_state().is_pp_last_stage or args.custom_advantage_function_path is None:
            return
        log_probs = []
        values = []
        rollout_data[log_probs_key] = log_probs
    ...
    if args.custom_advantage_function_path is not None:
        custom_advantage = load_function(args.custom_advantage_function_path)
        advantages, returns = custom_advantage(args, rollout_data, kl)
    else:
