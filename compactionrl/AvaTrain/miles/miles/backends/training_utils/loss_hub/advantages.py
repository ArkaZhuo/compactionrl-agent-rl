...
def normalize_advantages(
    ...
    if all_masks.numel() > 0:
        ...
        # CP ranks hold disjoint response shards. Whitening over intra_dp
        # would make each CP rank use statistics from only its local shard
        # (especially wrong for the production DP=1, CP=2 layout). Include
        # CP in the reduction group so every rank uses one global token
        # distribution while TP remains excluded.
        dp_group = parallel_state.intra_dp_cp.group
