...
def calculate_log_probs_and_entropy(
    ...
    def prepare_fp32(chunk: torch.Tensor) -> torch.Tensor:
        """Create the mutable fp32 buffer expected by fused vocab CE.

        Upcasting after chunking avoids materializing the complete packed
        vocabulary tensor in fp32.  This is numerically equivalent to the old
        full-tensor pointwise cast followed by slicing.
        """

        prepared = chunk.float()
        if temperature > 0 and temperature != 1.0:
            prepared = prepared.div(temperature)
        elif prepared is chunk:
            # fused_vocab_parallel_cross_entropy may modify its input in place.
    ...
    if logits.size(0) != 0:
        if chunk_size > 0:
            ...
            for tokens_chunk, logits_chunk in zip(tokens_chunks, logits_chunks, strict=True):
                log_prob = compute_log_probs(prepare_fp32(logits_chunk), tokens_chunk, tp_group)
                log_probs.append(log_prob)
            log_prob = torch.cat(log_probs, dim=0)
            if with_entropy:
                entropys = []
                for _, logits_chunk in zip(tokens_chunks, logits_chunks, strict=True):
                    logits_chunk = prepare_fp32(logits_chunk)
                    if entropy_requires_grad:
                        entropy = compute_entropy_from_logits(logits_chunk, tp_group)
                    else:
                        with torch.no_grad():
            ...
            prepared_logits = prepare_fp32(logits)
            log_prob = compute_log_probs(prepared_logits, tokens, tp_group)
            if with_entropy:
                if entropy_requires_grad:
                    entropy = compute_entropy_from_logits(prepare_fp32(logits), tp_group)
                else:
                    with torch.no_grad():
                        entropy = compute_entropy_from_logits(prepare_fp32(logits).detach(), tp_group)
    else:
        # Non-empty log-probs are computed from prepare_fp32(logits). Keep the
        # empty-response contract identical so a CP-local batch cannot mix the
        # model dtype with fp32 merely because some samples own zero tokens.
        log_prob = torch.zeros((0,), dtype=torch.float32, device=logits.device)
        if with_entropy:
            entropy = torch.zeros((0,), dtype=torch.float32, device=logits.device)
