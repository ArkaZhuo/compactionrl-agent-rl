...
def postprocess_rollout_data(args, data, train_parallel_config):
    ...
    has_compaction_samples = any(
        isinstance(getattr(sample, "train_metadata", None), dict)
        and sample.train_metadata.get("compaction") is True
        for sample in data
    )
    if has_compaction_samples:
        from compaction_swe.batching import align_compaction_trajectories

        alignment_divisor = train_parallel_config["dp_size"] * (args.num_steps_per_rollout or 1)
        alignment = align_compaction_trajectories(data, alignment_divisor)
        data = alignment.samples
        if alignment.removed_trajectory_ids:
            logger.warning(
                "Aligned CompactionRL batch by dropping %d complete segment(s) from %d trajectory(s): %s",
                alignment.removed_segments,
                len(alignment.removed_trajectory_ids),
                ",".join(alignment.removed_trajectory_ids),
            )
