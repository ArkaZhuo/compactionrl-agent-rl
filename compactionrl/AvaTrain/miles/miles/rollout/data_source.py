...
class RolloutDataSource(DataSource):
    ...
    def load(self, rollout_id=None):
        ...
        if getattr(self.args, "reset_rollout_dataset_state", False):
            logger.info(
                "Reset rollout dataset state for prompt_data=%s while resuming at rollout_id=%s",
                self.args.prompt_data,
                rollout_id,
            )
            return

        state_load = getattr(self.args, "rollout_dataset_state_load", None) or self.args.load
        if state_load is None:
            return

        path = os.path.join(state_load, f"rollout/global_dataset_state_dict_{rollout_id}.pt")
