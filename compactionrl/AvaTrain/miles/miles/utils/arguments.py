...
from miles.backends.sglang_utils.arguments import normalize_args as normalize_sglang_args
...
def get_miles_extra_args_provider(add_custom_arguments=None):
    def add_miles_arguments(parser):
        ...
        def add_data_arguments(parser):
            ...
            parser.add_argument(
                "--rollout-dataset-state-load",
                type=str,
                default=None,
                help=(
                    "Optional checkpoint root used only to restore rollout dataset state. "
                    "This supports critic-only warmup recovery where no actor checkpoint exists."
                ),
            )
            parser.add_argument(
                "--reset-rollout-dataset-state",
                action="store_true",
                help=(
                    "Start the prompt dataset cursor from the beginning while resuming model and optimizer state. "
                    "Use this only when intentionally switching to a different prompt dataset."
                ),
            )
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=int,
                default=None,
                help="Optional PPO epoch count for the critic only; default keeps --ppo-epochs for both roles.",
            )
            parser.add_argument(
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Optional function path receiving (args, rollout_data, kl) and returning "
                    "(advantages, returns). The default advantage implementation is unchanged."
                ),
            )
            parser.add_argument(
                "--compaction-gae-alpha",
                type=float,
                default=1.5,
                help="Alpha used by the opt-in length-adaptive CompactionRL GAE implementation.",
            )
            parser.add_argument(
                "--kl-loss-type",
                type=str,
...
def parse_args(add_custom_arguments=None):
    ...
    # Normalize version-dependent SGLang CLI fields before Miles reads them.
    # The full SGLang validation still runs below after Megatron validation.
