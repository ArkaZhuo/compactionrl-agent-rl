            ...
            for dtype, gbuf_range_map_for_all_buckets in gbuf_range_maps.items():
                ...
                for bucket_idx, gbuf_range_map in enumerate(gbuf_range_map_for_all_buckets):
                    ...
                    for model_param, param_range_map in gbuf_range_map["param_map"].items():
                        ...
                        # Adam's step is scalar, shared through optimizer param_groups,
                        # and must not be represented as parameter state. Keeping it
                        # here makes the padding code below expand it to the padding
                        # length; a later load can then replace the scalar step with
                        # that vector and make fused Adam fail at optimizer.step().
            ...
            for dtype, gbuf_range_map_for_all_buckets in gbuf_range_maps.items():
                for bucket_idx, gbuf_range_map in enumerate(gbuf_range_map_for_all_buckets):
                    ...
                    for src_tensors, (model_param, param_range_map) in zip(
                        ...
                        # Main param & optimizer states.
                        # Older HybridDeviceOptimizer checkpoints may contain a
                        # parameter-shaped `step` created from bucket padding. The
                        # correct scalar was already restored from param_groups in
                        # load_state_dict(), so never overwrite it here.
                        src_tensors = {k: v for k, v in src_tensors.items() if k != "step"}
