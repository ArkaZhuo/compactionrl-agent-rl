...
def merge(x1: Union[dict, list], x2: Union[dict, list], key: Tuple[Union[str, int], ...] = ()):
    ...
    For optimizer param_state paths, allows either list to be longer than the
    other. ``dp_reshardable`` stores common state but reconstructs padding
    entries locally, so a different local padding layout can make either side
    longer. Trailing checkpoint-only entries are truncated and trailing local
    ...
    if isinstance(x1, dict) and isinstance(x2, dict):
        ...
        if len(x1) != len(x2):
            if not _is_optimizer_param_state_key(key):
                raise ValueError(
                    f"Cannot merge two lists with different lengths ({len(x1)} and {len(x2)}, "
                    f"encountered at level {key})"
                )
            overlap = min(len(x1), len(x2))
            for i in range(overlap):
                x1[i] = merge(x1[i], x2[i], key=key + (i,))
            if len(x1) < len(x2):
                x1.extend(x2[overlap:])
            else:
                del x1[overlap:]
            return x1
